...
<form id="posts-form" method="POST" enctype="multipart/form-data" action="{{ \Request::route()->getName() == 'products.create'  ? route('products.store') : route('products.update', $product->id) }}">
    ...
    <div class="form-group">
        ...
        <input type="text" name="description" class="form-control" placeholder="Product Description" value="{{ $product->description ?? "" }}" >
...
<div class="form-group">
    ...
    <input type="number" step="any" name="price" class="form-control" placeholder="Product price" value="{{ $product->price ?? "" }}" >
    ...
    <div class="dropzone" id="media-dropzone" style="border: 2px dotted red"></div>
    </div>
<br>
<br>
...
<script>
    ...
    @if(Route::currentRouteName() == 'products.edit')
        ...
        editingPost = {!! json_encode($product->toArray(), JSON_HEX_TAG) !!};//json_hex_tag for security
